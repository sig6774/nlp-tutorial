...
class NNLM(nn.Module):
    ...
    def forward(self, X):
        ...
        # 각 단어 인덱시에 대해서 설명할 수 있는 정해진 차원의 벡터를 생성

        X = X.view(-1, n_step*m)
        # view 차원을 줄여줌 X : [batch_size, n_step*m]
        # self.H가 n_step*m이라서 그것에 맞도록 줄여준건감???
            # 각 단어를 2개의 벡터로 설명하기 때문에 n_step을 2로 해준건가
        # view를 사용해서 [3,2,2]였던 차원을 [3,4]로 변환
        # 즉, 각 문장에 target을 뺀 문장의 벡터를 구성
        # 근데 target을 맞추기 위한 단어의 개수가 달라지면 어떻게 되는거지
...
4. 학습을 거치면서 가중치 값을 조정하면서 타겟 단어의 인덱스를 맞추도록 함 
